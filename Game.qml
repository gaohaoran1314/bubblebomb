...
Rectangle {
    ...
    Keys.onPressed: function(event) {
        ...
        if (event.key === keyBindings.keyLeft) player1.keyLeft = true
        if (event.key === keyBindings.keyRight) player1.keyRight = true
        if (event.key === keyBindings.keyUp) player1.keyUp = true
        if (event.key === keyBindings.keyDown) player1.keyDown = true
    ...
    Component.onCompleted: {
        ...
        if (music && typeof music.playBGM === "function") {
            music.playBGM()
        }
    }
    ...
    function checkGameEnd() {
        ...
        if (mode === "single") {
            if (player1.isDead) { gameOver = true; victory = false
                // 播放失败音效
                if (music && typeof music.playLose === "function") music.playLose()}
    ...
    function returnToMainMenu() {
        ...
        if (music && typeof music.stopBGM === "function") {
            music.stopBGM()
        }
